# navigation using TD3. For Training
...
class Buffer:
  ...
  def update(
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      y = reward_batch + gamma * tf.math.minimum(target_critic([next_state_batch, target_actions], training=True),
                                      target_critic2([next_state_batch, target_actions], training=True))
      critic_value = critic_model([state_batch, action_batch], training=True)
      critic_value2 = critic_model2([state_batch, action_batch], training=True)

      critic_loss = tf.math.reduce_mean(tf.math.square(y - critic_value)+tf.math.square(y - critic_value2))

    critic_grad = tape.gradient(critic_loss, critic_model.trainable_variables)
    critic_grad2 = tape.gradient(critic_loss, critic_model2.trainable_variables)
    critic_optimizer.apply_gradients(zip(critic_grad, critic_model.trainable_variables))
    critic_optimizer.apply_gradients(zip(critic_grad2, critic_model2.trainable_variables))
...
for kk in range(2):
  ...
  for i in range(total_episodes):
    ...
    print("Episode * {} * Ep Reward is ==> {} * Avg Reward is ==> {}".format(i + 1, round(episodic_reward, 2),
                                                                                          round(avg_reward, 2)))
  actor_model.save_weights("fly_actor_TD3_{}.h5".format(1))
  ###########################
  ####### Euler ODE Bar #####
...
savemat("Reward_TD3_vanilla_01.mat", to_file)
