# Model-based Reinforcement Learning for navigation using VAE. For Training
...
from tensorflow.keras.layers import Dense, Flatten, Add, Activation, Input, experimental, Concatenate, Activation
...
class Buffer:
  ...
  def update(self, state_batch, action_batch, reward_batch, next_state_batch):
    ...
    with tf.GradientTape(persistent=True) as tape:
      encod = encoder_model(state_batch, training=True)
      encoder_value = encod[:, 0:2]
      pos = tf.stack([state_batch[:, 2], state_batch[:, 3]], axis=1)
      encoder_value2 = encoder_circ_model([encoder_value,pos], training=True)
      z_mean = encod[:, 2:5:2]
      z_log_sigma = encod[:, 3:6:2]
      decoder_value = decoder_model(encoder_value2, training=True)
      reconstruction_loss = num_states * tf.math.reduce_mean(tf.math.square(next_state_batch - decoder_value))
      Kl_loss = -0.5 * tf.reduce_sum(1 + z_log_sigma - tf.math.square(z_mean) - tf.math.exp(z_log_sigma), axis=-1)
      ##################################################

      divv = pos[:,0] / tf.norm(pos, axis=1)
      divv2 = pos[:,1] / tf.norm(pos, axis=1)
      ring_posterior = encoder_value / 20 + tf.stack([divv,divv2], axis=1)

      circ_loss = tf.math.reduce_mean(tf.math.square(ring_posterior - encoder_value2))
      vae_loss = reconstruction_loss + Kl_loss

    enc_grad = tape.gradient(vae_loss, encoder_model.trainable_variables)
    encoder_optimizer.apply_gradients(zip(enc_grad, encoder_model.trainable_variables))
    encc_grad = tape.gradient(circ_loss, encoder_circ_model.trainable_variables)
    encoder_circ_optimizer.apply_gradients(zip(encc_grad, encoder_circ_model.trainable_variables))
    dec_grad = tape.gradient(vae_loss, decoder_model.trainable_variables)
    decoder_optimizer.apply_gradients(zip(dec_grad, decoder_model.trainable_variables))

    with tf.GradientTape(persistent=True) as tape:
      encod = encoder_model(next_state_batch, training=True)
      encoder_value = encod[:, 0:2]
      pos = tf.stack([next_state_batch[:, 2], next_state_batch[:, 3]], axis=1)
      encoder_value2 = encoder_circ_model([encoder_value,pos], training=True)
      target_actions = target_actor([next_state_batch,encoder_value2], training=True)
      y = reward_batch + gamma * target_critic([next_state_batch,encoder_value2, target_actions], training=True)
      encod = encoder_model(state_batch, training=True)
      encoder_value = encod[:, 0:2]
      pos = tf.stack([state_batch[:, 2], state_batch[:, 3]], axis=1)
      encoder_value2 = encoder_circ_model([encoder_value,pos], training=True)
      critic_value = critic_model([state_batch,encoder_value2, action_batch], training=True)
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      pos = tf.stack([state_batch[:, 2], state_batch[:, 3]], axis=1)
      encoder_value2 = encoder_circ_model([encoder_value, pos], training=True)
      actions = actor_model([state_batch,encoder_value2], training=True)
      critic_value = critic_model([state_batch,encoder_value2, actions], training=True)
  ...
  def learn(self,pre):
    ...
    if pre == 1:
      self.updatepre(state_batch, action_batch, reward_batch, next_state_batch)
    else:
...
def get_critic():
  ...
  observation_latent = Input(shape=(2), name='observation_latent')
  action_input = Input(shape=(num_actions), name='action_input')
  crit = Concatenate(name='CriticL3')([observation_input,observation_latent,action_input])
  ...
  critModel = Model(inputs=[observation_input,observation_latent, action_input], outputs=crit_out, name='Critic')
  return critModel
###########################
##### Actor net ###########
def get_actor():
...
###########################
## Policy (sampling net) ##
def policy(state, laten, noise_object):
  sampled_actions = tf.squeeze(actor_model([state,laten]))
  noise = noise_object()
...
for kk in range(1):
  ...
  target_actor = policy_decoder()  # Target networks learn slowly to maintain stability
  target_critic = get_critic()

  encoder_model = encoder()
  encoder_circ_model = encoder_circ()
  decoder_model = decoder()
  poldec_model = policy_decoder()
  ...
  encoder_optimizer = tf.keras.optimizers.Adam(encdec_lr)
  encoder_circ_optimizer = tf.keras.optimizers.Adam(encdec_lr)
  decoder_optimizer = tf.keras.optimizers.Adam(encdec_lr)
  ...
  for i in range(total_episodes):
    ...
    latent = encoder_model(tf.expand_dims(tf.convert_to_tensor(prev_state), 0))
    ss = tf.expand_dims(tf.stack([prev_state[2],prev_state[3]]), axis=0)
    prelat = choose_state(latent, ss)
    while True:
      tf_prev_state = tf.expand_dims(tf.convert_to_tensor(prev_state), 0)
      #tf_prelat = tf.expand_dims(tf.convert_to_tensor(prelat), 0)
      action = policy(tf_prev_state, prelat, ou_noise)
      # Recieve state and reward from environment.
      state, reward, done, info, psi_fly, psi_fly_dot = env.step(action)
      latent = encoder_model(tf.expand_dims(tf.convert_to_tensor(state), 0))
      ss = tf.expand_dims(tf.stack([state[2],state[3]]), axis=0)
    ...
    print("Episode * {} * Ep Reward is ==> {} * Avg Reward is ==> {}".format(i + 1, round(episodic_reward, 2),
                                                                                          round(avg_reward, 2)))
  actor_model.save_weights("policy_{}.h5".format(1))
  encoder_model.save_weights("encoder_{}.h5".format(1))
  encoder_circ_model.save_weights("encoder_compass_{}.h5".format(1))
  decoder_model.save_weights("decoder_{}.h5".format(1))
  ###########################
  ####### Euler ODE Bar #####
  ...
  phi = 0
  lat_space_x = []; lat_space_y = []; state_label_Tor = []; state_label_F = []
  ...
  to_file["Dim1"].append(lat_space_x)
  to_file["Dim2"].append(lat_space_y)
  to_file["Label1"].append(state_label_Tor)
  to_file["PosX"].append(xy1)
  to_file["PosY"].append(yy1)
  to_file["VelX"].append(xy2)
  to_file["VelY"].append(yy2)
  to_file["Algorithm"].append("VAE-DDPG")

savemat("Reward_VAE-COMPASS_1.mat",  to_file)
...
for j in range(len(to_file["Trial"])):
  for i in range(len(to_file["Reward"][0])):
    ...
    rew_dic["Algorithm"].append("VAE-DDPG")
